...
3. Normalize the sensitivity metrics across principles
...
The following table the two reward models used and their respective performance on the [Reward Bench](https://huggingface.co/spaces/allenai/reward-bench) benchmark as of 2025-01-31.
...
After computing the reward values for the original and perturbed prompts, we create a dataset that contains the reward values for the original and perturbed prompts.
...
An elementary effect should technically be computed by independently perturbing one input variable at a time while holding the others constant, but in this case the prompts are perturbed according to a single principle at a time.
...
After computing the sensitivity metrics for each principle, the sensitivity metrics are sum normalized across principles to allow for comparison between the different principles.
...
A histogram of the rewards for the `Ray2333/GRM-Llama3.2-3B-rewardmodel-ft` model across all the evaluated query-response pairs from the hh-rlhf dataset are shown below.
...
However, it is important to recognize that the perturbation is performed using an aligned model, meaning the responses it produces are likely to highly reward (For example by producing well formatted text).
...
The results for the mean effects sensitivity metric for the two models evaluated are shown below.
...
The similarity in sensitivity metrics for the two models is likely due to the fact that both models were trained on the same preference dataset by the same author.[^same_author]
...
The most suitable sensitivity metric for characterising reward models would likely have to be identified by human evaluators who rate the effectiveness of the sensitivity metric after interacting with LLM's that have been RLHF'ed with different reward models.
...
- The method involves perturbing a dataset of prompts according to different principles through critique and revision, and measuring the effect on the reward model output.
...
- The two reward models that were used were trained by [the same author](https://arxiv.org/pdf/2406.10216) and the same dataset.
...
Sensitivities to different constitutions can possibly be used to regularise reward models such that the latent features associated with different constitutional principles are orthogonal to each other.
...
| David Marx      | Reading and commenting on the draft.                                                         | 
